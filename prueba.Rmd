...
renderHighchart(
    if(!is.null(input$Productor_m)){
        hchart(flujos_productor()%>%
            ...
            # ddply(.(Fecha, Presentacion), summarize, Saldo = sum(Total)), 
              arrange(Fecha)%>%
             group_by(Presentacion)%>%
              mutate(Saldo = cumsum(Total))%>%
              ungroup(),
             hcaes(x = Fecha, y = Saldo, group = Presentacion), type = "line")%>%
           hc_chart(zoomType = "xy", panKey = "ctrl", panning = TRUE)
...
renderDataTable(datatable(flujos_productor()%>%
                            ddply(.(Tipo_material, Presentacion), summarize, Total = sum(Total))))
```
